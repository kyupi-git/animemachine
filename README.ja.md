...
*ライブラリ概要：新作レーダーとライブラリは、確認済みの新エピソード更新時刻を並べ替え証拠として共有します。Ani-RSS が利用できない場合はランダム順へ自動的にフォールバックします。*

![AnimeMachine 作品詳細](docs/images/work-detail.png)

*作品詳細：Bangumi Archive の主要メタデータに Ani-RSS と実メディア状態を組み合わせ、話数進捗、所蔵、再生元を表示します。*
...
- Bangumi Archive を基礎にアニメカタログを構築します。初回起動時はローカル SQLite Catalog をバックグラウンドで作成し、完全性検査後に原子的に公開します。 取込・定期走査では中間データとキャッシュを制限し、索引を必要時だけ生成して初回構築と継続運用のメモリ負荷を抑えます。
...
- 「新作レーダー」は前・今・次シーズンの表示と「新作追従」の並べ替えを統合します。Ani-RSS が正常な間は確認済みの新話進行を新しい順に並べ、障害時はランダム順へ退避し、未購読作品でも後からより大きい話数を観測すれば更新時刻を進めます。タイトルは現在の UI 言語に従い、「タイトル」と「最新 / 総話数」の間に「メディア種別」を独立列として表示し、全セルを一行表示のままタイトル幅を優先します。新しく更新された作品はハイライトされ、クリックすると既読になり、その作品の更新時刻が次に進むまで再ハイライトしません。映画はローカル Bangumi Archive infobox の意味が明確な劇場公開・オンライン配信・BD/Blu-ray 日付でも四半期へ入り、`subject.date` や一般的な発売／リリース欄を BD 日付として扱わず、Archive 用意後は作品単位のオンライン照会を行いません。作品詳細とレーダーの話数はシリーズ通算ではなく本作の「現在の最新話 / 既知の総話数」とし、連続通算番号を使うシリーズでは Archive の本作開始話数から現在のクール／編の番号へ変換します。分割クール／付属作品が一つの物理ディレクトリを共有しても、追従話数・更新時刻・Ani-RSS の購読／リソース状態は論理作品ごとに分離し、個別購読行にも同じ本作番号変換を適用します。Archive で既知の本作総話数は Ani-RSS のシリーズ通算総数より優先し、実際の本作進捗が Archive 件数を超えた場合だけ例外とします。ライブラリは実メディア在庫と Ani-RSS リモート再生を分離し、「利用可能なソース」は Torrent、Ani-RSS、「利用可能なソースなし」を既定ですべて選択し、「すべてリセット」でも三項目へ戻します。
- リモート Ani-RSS の利用可否は「現在の資格情報 + エンドポイント + 実際のプロキシ／直接接続ルート」で判定します。プロキシまたは `NO_PROXY` が変わると、以前の `ready` スナップショットはいったん現在のリソース／再生元から外れ、補償同期で再検証された後に自動復帰します。Ani-RSS が利用不能な間も Torrent、Bangumi 画像、ローカルライブラリ、独立マウントした読み取り専用メディアは継続し、ローカル／LAN の Ani-RSS は常に直接接続します。正常な同期で作品に明示的な Ani-RSS カバーが確認できた場合は、従来画像ソースが残した `no_cover` の負キャッシュも解除し、カバー保守周期を待たず次の通常画像要求から Ani-RSS を利用できます。 Ani-RSS のエンドポイントまたは API Key を切り替えた場合は、旧インスタンスのリソース検索キャッシュも即時に無効化します。旧ソースで実行中だった検索結果が遅れて到着してもプロバイダー世代不一致として破棄し、新しいソースの再検証後はリソース探索を直ちに期限到達として再実行できます。リモートのリソース検索は開始時の実効プロキシルート世代も固定し、検索中にプロキシまたは `NO_PROXY` が世代交代した場合はコミット前にその結果を破棄するため、遅い要求がネットワーク世代をまたいで書き戻されません。
...
- 作品関係図でシリーズ内の位置と論理関係を表示し、関連作品へ直接移動できます。ノード名はカード／詳細画面と同じ現在の UI 言語規則を使います。Catalog のタイトル方針は保存済み英語別名を自動再評価し、英語として誤分類された非ラテン文字を含む別名（中国語・日本語・韓国語・キリル・ギリシャ・アラビア文字など）を除外して、検証可能な英語表示名を Catalog 再構築なしで補正します。上流に英語名が実際に存在しない場合、英語 UI は使いやすさを優先して作品の原題を一時的に表示しますが、その原題を英語名として保存・分類しません。検証済み英語名を後から取得すれば自動的にそちらを優先します。
...
0.3.0 の構成例は公開イメージ `ghcr.io/kyupi-git/animemachine:0.3.0` を既定で固定します。完全 Compose 構成では Ani-RSS `v3.2.28` と qBittorrent `5.2.3` も固定し、現在のアダプターはこの二つのインターフェースで検証済みです。外部 qBittorrent は引き続き 5.2.0 以降が必要です。`latest` へ変更すると以後のリリースへ追従するため、運用環境では具体的なバージョンタグを固定したまま使うことを推奨します。
