...
*收藏库首页：新作雷达与收藏库共用已确认的新集更新时间作为排序证据；Ani-RSS 不可用时自动回退到随机排序。*

![AnimeMachine 作品详情页](docs/images/work-detail.png)

*作品详情页：以 Bangumi Archive 元数据为基础，并结合 Ani-RSS 与实际媒体状态展示集数、收藏和播放来源。*
...
- 基于 Bangumi Archive 的动画底库。首次启动会在后台构建本地 SQLite Catalog，完整性检查通过后再原子发布。 导入与周期扫描会限制中间数据和缓存占用，并按需构建索引，降低首次建库和长期运行的内存压力。
...
- “新作雷达”集中展示上/本/下季度的 TV 与剧场版/动画电影，并整合“新作追更”排序：Ani-RSS 健康时按已确认的新集证据从近到远排列，断连时自动退回随机排序；未订阅作品后续发现更大集数也会推进更新时间。雷达标题按当前界面语言显示，在标题与“最新 / 总集数”之间单列“媒介类型”，所有单元格保持单行并优先保留标题宽度；新集更新会高亮，点击该作品后保持已读，直到下一次更新时间推进才重新高亮。电影可依据本地 Bangumi Archive infobox 中语义明确的上映、网络上线或 BD/Blu-ray 日期进入相应季度，`subject.date` 与泛化发售/发行字段不会被当作 BD 日期，Archive 就绪后无需逐作品联网。作品详情与雷达的集数统一为本作“当前最新 / 已知总集数”，并利用 Archive 的本作首集编号把系列累计编号换算为当前季度/篇章编号；即使分 cour/附属作品共用同一物理目录，追更集数、更新时间和 Ani-RSS 订阅/资源状态仍按逻辑作品隔离，详情中的单条订阅也显示本作换算后的话数；Archive 已知本作总集数优先于 Ani-RSS 的系列级总数，除非实际本作进度已经超过 Archive 计数。收藏库列表继续把实际媒体库存与 Ani-RSS 远程播放通道分开；“可用来源”默认同时勾选 Torrent、Ani-RSS 和“无可用来源”，重置全部也恢复三项全选。
- 远程 Ani-RSS 采用“当前凭据 + 端点 + 实际代理/直连路由”健康门控：代理或 `NO_PROXY` 变化后旧 `ready` 快照立即暂时退出当前资源/播放来源，并触发补偿同步；重新验证成功后自动恢复。Ani-RSS 不可用期间 Torrent、Bangumi 图片、本地收藏与独立挂载的只读媒体仍继续工作，本地/LAN Ani-RSS 始终直连。成功同步若发现 Ani-RSS 已为作品提供明确封面，还会解除此前由自有图片源留下的 `no_cover` 负缓存，使下一次正常图片请求即可转用 Ani-RSS，而无需等待图片维护周期。 切换 Ani-RSS 端点或 API Key 时，上一实例的资源搜索缓存会立即失效；旧实例仍在执行的搜索即使晚到也会因来源代次不匹配而丢弃，新实例重新验证后资源扫描立即重新到期，避免旧候选短暂重新出现。远程资源搜索还会固定其开始时的有效代理路由代次；若代理或 `NO_PROXY` 在搜索途中换代，本轮结果会在写入前被丢弃，避免慢请求跨网络环境回灌。
...
- 支持生成作品关系图，呈现动画在所属系列的位置，系列中各作品的逻辑关系。在作品关系图中，可以点击跳转其它关联作品；节点标题与卡片、详情页使用同一套当前界面语言规则。Catalog 标题策略会自动重新核对已保存的英文别名，剔除任何含非拉丁字母、却被误标为英文的别名（包括中/日/韩、西里尔、希腊、阿拉伯等文字）并补齐可验证的英文显示标题；上游确实没有英文名时，英语界面会临时回退显示作品原标题，但不会把该原标题写入或标记为英文标题；一旦后续取得可靠英文名会自动优先使用，无需重建 Catalog。
...
0.3.0 的预设方案默认固定公开镜像 `ghcr.io/kyupi-git/animemachine:0.3.0`。完整 Compose 同时固定 Ani-RSS `v3.2.28` 与 qBittorrent `5.2.3`，当前适配层已按这两版接口核验；外接 qBittorrent 继续要求 5.2.0 或更新版本。若自行改为 `latest`，Compose 会跟随后续发布；生产环境建议继续固定具体版本标签。
