...
*Library overview: Release radar and the library share confirmed new-episode timestamps as ordering evidence; when Ani-RSS is unavailable, the UI falls back to random ordering.*

![AnimeMachine work detail](docs/images/work-detail.png)

*Work detail: principal Bangumi Archive metadata combined with Ani-RSS and actual-media state for episode progress, inventory, and playback sources.*
...
- Release radar combines the previous/current/next-season view with New Episode Follow-up ordering. While Ani-RSS is healthy, confirmed episode advances are ordered newest-first; outages fall back to random ordering, and later larger episode observations can also promote unsubscribed works. Titles follow the interface language, Media type is a separate column between Title and Latest / total episodes, all cells stay on one line, and title width is protected. A newly updated work is highlighted; clicking it marks that update as seen until its update timestamp advances again. Films can enter a quarter from semantically explicit theatrical, online-platform, or BD/Blu-ray dates in the local Bangumi Archive infobox; `subject.date` and generic Sale/Release fields are not treated as BD dates, and no per-work lookup is needed once the Archive is available. Work details and radar report progress for the current work rather than a franchise-wide cumulative number, using the Archive first-episode number when a series uses continuous numbering. Even when split cours or supplements share one physical directory, follow-up episodes, update timestamps, and Ani-RSS subscription/resource state remain isolated by logical work; individual subscription rows use the same work-local conversion. A known Archive work total takes precedence over a franchise-wide Ani-RSS total unless observed local progress has already exceeded the Archive count. The library continues to separate actual media inventory from Ani-RSS remote playback; Available sources defaults to Torrent, Ani-RSS, and No available source all selected, and Reset all restores all three.
- A remote Ani-RSS source is gated on the current credentials, endpoint, and effective proxy/direct route. A proxy or `NO_PROXY` change immediately makes the old `ready` snapshot temporarily unavailable for current resource/playback use and triggers compensating synchronization; successful revalidation restores it automatically. Torrent handling, Bangumi images, the local library, and independently mounted read-only media continue while Ani-RSS is unavailable, and local/LAN Ani-RSS remains direct. When a successful synchronization finds an explicit Ani-RSS cover for a work, it also releases any earlier `no_cover` negative cache left by AnimeMachine’s own image sources, allowing the next normal image request to use Ani-RSS without waiting for the cover-maintenance cadence. Changing the Ani-RSS endpoint or API key also invalidates the previous instance’s resource-search cache immediately; a late result from an in-flight search on the old source is discarded by provider-generation checks, and resource discovery becomes due immediately after the replacement source is revalidated. A remote resource search also captures its effective proxy-route generation at start; if the proxy or `NO_PROXY` changes while that search is in flight, its result is discarded before commit so a slow request cannot write across network generations.
