...
if [ -f "$root/compose.yaml" ] && grep -Eq '^[[:space:]]+(qbittorrent|ani-rss):' "$root/compose.yaml"; then
...
chmod 600 "$root/.env"
if [ -z "$had_admin" ] || [ -z "$had_qbt_web" ] || [ -z "$had_ani_key" ]; then
  ...
  [ -n "$had_qbt_web" ] || printf '  qBittorrent  user=%s  password=%s\n' "$(value ANM_QBT_WEB_USERNAME)" "$(value ANM_QBT_WEB_PASSWORD)"
  [ -n "$had_ani_key" ] || printf '  Ani-RSS      API key=%s\n' "$(value ANM_ANI_RSS_API_KEY)"
